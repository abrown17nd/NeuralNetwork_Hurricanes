...
To view the results of this training, run the LSTM_Model_dropsondes_viewer.py file which will create predicted output using the 16 trained models with a random dropsonde header as the starting seed values. 

Instead of one single model, in this part there are the 16 models using a variety of hidden layer and sequence length sizes: hidden_sizes = [8, 16, 32, 64],  sequence_lengths = [5, 10, 20, 25], giving 16 different best models.
